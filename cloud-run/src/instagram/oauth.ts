...
export async function consumeOAuthState(state: string) {
  ...
  return db.runTransaction(async tx => { const snapshot = await tx.get(ref); const value = snapshot.data(); if (!snapshot.exists || !value || value.expiresAt < Date.now()) throw new Error("Invalid or expired OAuth state"); tx.delete(ref); return value.userId as string; });
}
...
export async function exchangeCode(code: string) {
  ...
  if (!shortResponse.ok || !short.access_token) throw new Error(short.error_message || "OAuth token exchange failed");
